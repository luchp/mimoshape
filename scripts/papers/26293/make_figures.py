...
def table_crest_benchmark():
    ...
    write_text_checked(TABLES_DIR / "crest_bench_stats.tex", 
        ...
        f"\\newcommand{{\\crestbenchtimes}}{{{min(times):.1f}\\; \\mathrm{{to}}\\; {max(times):.0f}}}\n"
    )
...
def fig_title_art():
    ...
    blocks.  Replaces the copyrighted Pollock reproduction.
    To make the curve self-similar, the diagonal CSD should be to be 1/f**b
    """
    nt = 2**13
    nf = nt // 2 + 1
